...
class FlowLauncherAPI:
    ...
    async def show_message(self, title: str, text: str, icon: str = "", use_main_window_as_owner: bool = True) -> Result:
        res = await self.jsonrpc.request("ShowMsg", [title, text, icon, use_main_window_as_owner])
        assert isinstance(res, Result)
        return res
